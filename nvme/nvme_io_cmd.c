...
void handle_nvme_io_read(unsigned int cmdSlotTag, NVME_IO_COMMAND *nvmeIOCmd)
{
	...
	unsigned int nlb, fua, customCmd;
	...
	ReqTransNvmeToSlice(cmdSlotTag, startLba[0], nlb, IO_NVM_READ, fua, customCmd);
}
...
void handle_nvme_io_write(unsigned int cmdSlotTag, NVME_IO_COMMAND *nvmeIOCmd)
{
	...
	ReqTransNvmeToSlice(cmdSlotTag, startLba[0], nlb, IO_NVM_WRITE, fua, customCmd);
}

void handle_nvme_io_write_read(unsigned int cmdSlotTag, NVME_IO_COMMAND *nvmeIOCmd)
{
	IO_READ_COMMAND_DW12 writeInfo12;
	//IO_READ_COMMAND_DW13 writeInfo13;
	//IO_READ_COMMAND_DW15 writeInfo15;
	IO_READ_COMMAND_DW12 readInfo13;
	unsigned int writeStartLba[2], readStartLba[2];
	unsigned int nlbWrite, nlbRead, fua, customCmd;

	writeInfo12.dword = nvmeIOCmd->dword[12];
	//writeInfo13.dword = nvmeIOCmd->dword[13];
	//writeInfo15.dword = nvmeIOCmd->dword[15];

	writeStartLba[0] = nvmeIOCmd->dword[10];
	writeStartLba[1] = nvmeIOCmd->dword[11];
	nlbWrite = writeInfo12.NLB;
	fua = writeInfo12.FUA;
	customCmd = 1;

	ASSERT(writeStartLba[0] < storageCapacity_L && (writeStartLba[1] < STORAGE_CAPACITY_H || writeStartLba[1] == 0));
	//ASSERT(nlb < MAX_NUM_OF_NLB);
	ASSERT((nvmeIOCmd->PRP1[0] & 0xF) == 0 && (nvmeIOCmd->PRP2[0] & 0xF) == 0);
	ASSERT(nvmeIOCmd->PRP1[1] < 0x10000 && nvmeIOCmd->PRP2[1] < 0x10000);

	readInfo13.dword = nvmeIOCmd->dword[13];
	readStartLba[0] = nvmeIOCmd->dword[14];
	readStartLba[1] = nvmeIOCmd->dword[15];
	nlbRead = readInfo13.NLB;

	/*readStartLba[0] = nvmeIOCmd->dword[10];
	readStartLba[1] = nvmeIOCmd->dword[11];
	nlbRead = 0;*/

	//ASSERT(readStartLba[0] < storageCapacity_L && (readStartLba[1] < STORAGE_CAPACITY_H || readStartLba[1] == 0));
	//ASSERT((nvmeIOCmd->PRP1[0] & 0xF) == 0 && (nvmeIOCmd->PRP2[0] & 0xF) == 0);
	//ASSERT(nvmeIOCmd->PRP1[1] < 0x10 && nvmeIOCmd->PRP2[1] < 0x10);

	ReqTransNvmeToSlice(cmdSlotTag, writeStartLba[0], nlbWrite, IO_NVM_WRITE, fua, customCmd);
	ReqTransNvmeToSlice(cmdSlotTag, readStartLba[0], nlbRead, IO_NVM_READ, fua, customCmd);
}
...
//			xil_printf("IO Read Command\r\n");
		...
		case IO_NVM_WRITE_READ:
		{
			//xil_printf("IO Custom Command\r\n");
			handle_nvme_io_write_read(nvmeCmd->cmdSlotTag, nvmeIOCmd);
			break;
		}
		default:
		{
