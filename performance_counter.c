...
void InitPerformanceCounter(unsigned char initOption)
{
	...
	performanceCounter->dataBufHitCnt = 0;
...
void UpdatePerformanceCounterForDataBufHit()
{
	performanceCounter->dataBufHitCnt++;
}
...
void AddLogForPerformanceCounter(XTime currentTimerClk)
{
	...
	dataBufHitCnt = performanceCounter->dataBufHitCnt;
	readModifyWriteCnt = performanceCounter->readModifyWriteCnt;

	xil_printf("SsdRead %dMB SsdWrite %dMB NandRead %dMB NandWrite %dMB EraseCnt %d\r\n",
			megaBytesPerSecForSsdRead, megaBytesPerSecForSsdWrite, megaBytesPerSecForNandRead, megaBytesPerSecForNandWrite, erasedBlockCnt);
}