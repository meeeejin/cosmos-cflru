...
CUSTOM_CMD_REQUEST_QUEUE customCmdQ;

unsigned int notCompletedNandReqCnt;
unsigned int notCompletedCustomCmdCnt;
...
void InitReqPool()
{
	...
	customCmdQ.headReq = REQ_SLOT_TAG_NONE;
	customCmdQ.tailReq = REQ_SLOT_TAG_NONE;
	customCmdQ.reqCnt = 0;
...
void PutToBlockedByBufDepReqQ(unsigned int reqSlotTag)
{
	...
	if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd == REQ_OPT_CUSTOM_CMD_ON)
		xil_printf("Push BlockedByBufDepReqQ reqSlotTag:%d Cnt:%d \r\n", reqSlotTag, blockedByBufDepReqQ.reqCnt);
#endif
}
...
void SelectiveGetFromNvmeDmaReqQ(unsigned int reqSlotTag)
{
	...
	UpdatePerformanceCounterForCompletedNvmeDmaReq(reqSlotTag);
}
...
void GetFromNandReqQ(unsigned int chNo, unsigned int wayNo, unsigned int reqStatus, unsigned int reqCode)
{
	...
	if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd == REQ_OPT_CUSTOM_CMD_ON)
		CompleteNandReqforCustomIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqSlotTag, reqCode);

	PutToFreeReqQ(reqSlotTag);
	ReleaseBlockedByBufDepReq(reqSlotTag);
}

void PutToCustomCmdReqQ(unsigned int reqSlotTag)
{
	if(customCmdQ.tailReq != REQ_SLOT_TAG_NONE)
	{
		reqPoolPtr->reqPool[reqSlotTag].prevReq = customCmdQ.tailReq;
		reqPoolPtr->reqPool[reqSlotTag].nextReq = REQ_SLOT_TAG_NONE;
		reqPoolPtr->reqPool[customCmdQ.tailReq].nextReq = reqSlotTag;
		customCmdQ.tailReq = reqSlotTag;
	}
	else
	{
		reqPoolPtr->reqPool[reqSlotTag].prevReq = REQ_SLOT_TAG_NONE;
		reqPoolPtr->reqPool[reqSlotTag].nextReq = REQ_SLOT_TAG_NONE;
		customCmdQ.headReq = reqSlotTag;
		customCmdQ.tailReq = reqSlotTag;
	}

	reqPoolPtr->reqPool[reqSlotTag].reqQueueType = REQ_QUEUE_TYPE_CUSTOM_CMD;
	customCmdQ.reqCnt++;
	notCompletedCustomCmdCnt++;

#if (PRINT_DEBUG_MSG == 1)
	xil_printf("Push CustomCmdQ reqSlotTag:%d Cnt:%d \r\n", reqSlotTag, customCmdQ.reqCnt);
#endif
}

void SelectiveGetFromCustomCmdReqQ(unsigned int reqSlotTag)
{
	unsigned int prevReq, nextReq;
	...
	{
		reqPoolPtr->reqPool[prevReq].nextReq = REQ_SLOT_TAG_NONE;
		customCmdQ.tailReq = prevReq;
	}
	else if((nextReq != REQ_SLOT_TAG_NONE) && (prevReq == REQ_SLOT_TAG_NONE))
	{
		reqPoolPtr->reqPool[nextReq].prevReq = REQ_SLOT_TAG_NONE;
		customCmdQ.headReq = nextReq;
	}
	else
	{
		customCmdQ.headReq = REQ_SLOT_TAG_NONE;
	...
	if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_RxDMA)
		CompleteRxforCustomIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqSlotTag, reqPoolPtr->reqPool[reqSlotTag].reqCode);
	if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_TxDMA)
		CompleteTxforCustomIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqSlotTag, reqPoolPtr->reqPool[reqSlotTag].reqCode);
...
#if (PRINT_DEBUG_MSG == 1)
	xil_printf("Pop CustomCmdQ reqSlotTag:%d Cnt:%d \r\n", reqSlotTag, customCmdQ.reqCnt);
#endif
}
