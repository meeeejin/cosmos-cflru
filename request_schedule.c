	...
	switch(dieStateTablePtr->dieState[chNo][wayNo].dieState)
	{
		...
		case DIE_STATE_EXE:
			if(reqStatus == REQ_STATUS_DONE)
			{
				...
				{
					UpdatePerformanceCounterForCompletedNandReq(reqSlotTag);
			...
			else if(reqStatus == REQ_STATUS_FAIL)
			{
				if((reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_READ) || (reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_READ_TRANSFER))
					if(retryLimitTablePtr->retryLimit[chNo][wayNo] > 0)
					{
						...
						UpdatePerformanceCounterForRetriedNandReq(reqSlotTag);
						return;
					}
				...
				GetFromNandReqQ(chNo, wayNo, reqStatus, reqPoolPtr->reqPool[reqSlotTag].reqCode);
				dieStateTablePtr->dieState[chNo][wayNo].dieState = DIE_STATE_IDLE;
				UpdatePerformanceCounterForFailedNandReq(reqSlotTag);
			}
...
void PrintDataBuffer(unsigned int reqSlotTag, unsigned int numOfDataBlock)
{
#if (PRINT_DATA_BUFFER == 1)
	unsigned int BUFFER_CNT = 1024;
	unsigned int devAddr = GenerateDataBufAddr(reqSlotTag);
	unsigned int dmaIndex = reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.startIndex;
	//unsigned int numOfDataBlock = reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.numOfNvmeBlock + 1;

	unsigned int i, j, k, bufIndex;

	typedef struct _TEMP_DATA_BUFFER
	{
		unsigned int data_buffer[BUFFER_CNT * numOfDataBlock];
	}TEMP_DATA_BUFFER, *P_TEMP_DATA_BUFFER;

	P_TEMP_DATA_BUFFER printDataBuffer = (P_TEMP_DATA_BUFFER)devAddr;
	xil_printf("Print devAddr:%08x\r\n", devAddr);
	for(i = 0; i < numOfDataBlock; i++)
	{
		xil_printf("nvmeBlock:%d\r\n", i);
		for(j = 0; j < BUFFER_CNT; j += k)
		{
			xil_printf(" ");
			for(k = 0; k < 8; k++)
			{
				bufIndex = (BUFFER_CNT*i) + j+k;
				xil_printf("%08x ", printDataBuffer->data_buffer[bufIndex]);
			}
			xil_printf("\r\n");
		}
	}
