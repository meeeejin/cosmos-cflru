...
#ifdef PRINT_READ_STALL_LATENCY
#include "xtime_l.h"
#endif /* PRINT_READ_STALL_LATENCY */
...
void ReqTransNvmeToSlice(unsigned int cmdSlotTag, unsigned int startLba, unsigned int nlb, unsigned int cmdCode, unsigned int fua, unsigned int customCmd)
{
	...
	if(customCmd == REQ_OPT_CUSTOM_CMD_ON)
	{
#if (PRINT_DEBUG_MSG == 1)
		xil_printf("CUSTOM CMD cmdSlotTag:%d StartLba:%d NvmeBlock:%d nlb: %u cmdcode: %d\r\n", cmdSlotTag, startLba, requestedNvmeBlock, nlb, cmdCode);
#endif
	}
	else
	{
#if (PRINT_DEBUG_MSG == 1)
		//xil_printf("IO CMD cmdSlotTag:%d StartLba:%d NvmeBlock:%d\r\n", cmdSlotTag, startLba, requestedNvmeBlock);
#endif
	}
	...
	reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd = customCmd;
...
void EvictDataBufEntry(unsigned int originReqSlotTag)
{
	...
	if(dataBufMapPtr->dataBuf[dataBufEntry].dirty == DATA_BUF_DIRTY)
	{
		...
		reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd = reqPoolPtr->reqPool[originReqSlotTag].reqOpt.customCmd;
		...
		if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd == REQ_OPT_CUSTOM_CMD_ON)
			AllocateNotCompletedNandReqforCustomIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqSlotTag, GenerateDataBufAddr(reqSlotTag));
...
void ReqTransSliceToLowLevel()
{
	...
	while(sliceReqQ.headReq != REQ_SLOT_TAG_NONE)
	{
...
#if (PRINT_DEBUG_MSG == 1)
			//if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd == REQ_OPT_CUSTOM_CMD_ON)
				//xil_printf("Hit data buffer reqSlotTag:%d\r\n", reqSlotTag);
#endif
			UpdatePerformanceCounterForDataBufHit();
		}
		else
		{
#ifdef PRINT_READ_STALL_LATENCY
			if (reqPoolPtr->reqPool[reqSlotTag].reqCode  == REQ_CODE_READ)	XTime_GetTime(&tStart);
#endif /* PRINT_READ_STALL_LATENCY */
...
#if (PRINT_DEBUG_MSG == 1)
			//if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd == REQ_OPT_CUSTOM_CMD_ON)
				//xil_printf("Miss data buffer reqSlotTag:%d\r\n", reqSlotTag);
#endif
...
#ifdef PRINT_READ_STALL_LATENCY
			if (reqPoolPtr->reqPool[reqSlotTag].reqCode  == REQ_CODE_READ)	XTime_GetTime(&tEnd);
			printf("%u Output took %llu clock cycles and %.2f us\n", reqSlotTag, 2 * (tEnd - tStart), 1.0 * (tEnd - tStart) / (COUNTS_PER_SECOND / 1000000));
#endif /* PRINT_READ_STALL_LATENCY */
		...
		if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd == REQ_OPT_CUSTOM_CMD_ON)
		{
			...
			{
				//**********Allocate Nand request//**********
				//EvictDataBufEntry(reqSlotTag);
				//**********//**********//**********//**********

				AllocateNotCompletedRxforCustomIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, GenerateDataBufAddr(reqSlotTag));
			}
			else if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_TxDMA)
			{
				//**********Allocate garbage data buffer//**********
				//dataBufEntry = AllocateDataBuf();
				//reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry = dataBufEntry;
				//**********//**********//**********//**********

				AllocateNotCompletedTxforCustomIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, GenerateDataBufAddr(reqSlotTag));
				AllocateNotIssuedTxforCustomIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, GenerateDataBufAddr(reqSlotTag));
			}
		}
...
void ReleaseBlockedByBufDepReq(unsigned int reqSlotTag)
{
	...
	if((targetReqSlotTag != REQ_SLOT_TAG_NONE) && (reqPoolPtr->reqPool[targetReqSlotTag].reqQueueType == REQ_QUEUE_TYPE_BLOCKED_BY_BUF_DEP))
	{
		...
		if(reqPoolPtr->reqPool[targetReqSlotTag].reqType == REQ_TYPE_NVME_DMA)
		{
			...
			if(reqPoolPtr->reqPool[targetReqSlotTag].reqOpt.customCmd == REQ_OPT_CUSTOM_CMD_ON)
			{
				PutToCustomCmdReqQ(targetReqSlotTag);
			}
			else
			{
...
void IssueNvmeDmaReq(unsigned int reqSlotTag)
{
	...
	if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_RxDMA)
	{
		while(numOfNvmeBlock < reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.numOfNvmeBlock)
		{
			...
			else if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd == REQ_OPT_CUSTOM_CMD_ON)
			{
#if (PRINT_DEBUG_MSG == 1)
	xil_printf("Issue Rx Nvme Dma reqSlotTag:%d devAddr:%08x\r\n", reqSlotTag, devAddr);
#endif
				set_auto_rx_dma(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, dmaIndex, devAddr, NVME_COMMAND_AUTO_COMPLETION_OFF);
			}
			else if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.forceUnitAccess == REQ_OPT_FORCE_UNIT_ACCESS_OFF
	...
	else if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_TxDMA)
	{
		while(numOfNvmeBlock < reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.numOfNvmeBlock)
		{
			...
			{
				if(customIoCmdCompleteTablePtr->customIoCmdCompleteEntry[reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag].notCompletedRxCount == 0)
				{
#if (PRINT_DEBUG_MSG == 1)
	xil_printf("Issue Tx Nvme Dma reqSlotTag:%d devAddr:%08x\r\n", reqSlotTag, devAddr);
#endif
					set_auto_tx_dma(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, dmaIndex, devAddr, NVME_COMMAND_AUTO_COMPLETION_OFF);
					if(numOfNvmeBlock == reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.numOfNvmeBlock - 1)
						IssuedTxforCustomIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqSlotTag, reqPoolPtr->reqPool[reqSlotTag].reqCode);
				}
				else
				{
#if (PRINT_DEBUG_MSG == 1)
	xil_printf("Failure Issue Tx Nvme Dma reqSlotTag:%d devAddr:%08x\r\n", reqSlotTag, devAddr);
#endif
					return;
				}
			}
...
{
	unsigned int reqSlotTag, prevReq, cmdSlotTag;
...
{
	unsigned int reqSlotTag, prevReq, cmdSlotTag;
	unsigned int rxDone, txDone, issueTxFlag, bufDepCheckReport;

	reqSlotTag = customCmdQ.tailReq;
	rxDone = 0;
	txDone = 0;
	...
	while(reqSlotTag != REQ_SLOT_TAG_NONE)
	{
		...
		if(reqPoolPtr->reqPool[reqSlotTag].reqCode  == REQ_CODE_RxDMA)
		{
			if(!rxDone)
				rxDone = check_auto_rx_dma_partial_done(reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.reqTail , reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.overFlowCnt);
			if(rxDone)
				SelectiveGetFromCustomCmdReqQ(reqSlotTag);
		}
		else //reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_TxDMA
		{
			if(customIoCmdCompleteTablePtr->customIoCmdCompleteEntry[cmdSlotTag].notCompletedRxCount == 0)
			{
				if(customIoCmdCompleteTablePtr->customIoCmdCompleteEntry[cmdSlotTag].notIssuedTxCount == 0)
				{
					if(!txDone)
						txDone = check_auto_tx_dma_partial_done(reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.reqTail , reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.overFlowCnt);
					if(txDone)
				...
				else if(customIoCmdCompleteTablePtr->customIoCmdCompleteEntry[cmdSlotTag].notIssuedTxCount > 0)
				{
					bufDepCheckReport = CheckBufDep(reqSlotTag);
